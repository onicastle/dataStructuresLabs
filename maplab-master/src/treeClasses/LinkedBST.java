...
implements BinarySearchTree<K, V>  {
	...
	private Position<Entry<K, V>> findGreatestPosition(Position<Entry<K, V>> r) {
		while (right(r) != null) 
			r = right(r); 
		return r;
	}
	...
	public Entry<K, V> removeMax() {
		Entry<K, V> temp = this.max();

		super.remove(this.findGreatestPosition(this.root()));
		return temp;
	}

	
	public Entry<K, V> removeMin() {
		Entry<K, V> temp = this.min();
		Position<Entry<K, V>> toBeRemoved = this.findSmallestPosition(this.root());
	...
	public Entry<K, V> max(){
		...
		return this.findGreatestPosition(this.root()).getElement();
