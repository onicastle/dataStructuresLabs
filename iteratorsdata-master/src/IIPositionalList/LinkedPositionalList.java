...
public class LinkedPositionalList<E> implements PositionalList<E> {
	...
	public LinkedPositionalList(L2FIteratorMaker<Integer> L2FIteratorMaker) {
		header = new DNode<>(); 
		trailer = new DNode<>(); 
		header.setNext(trailer);
		trailer.setPrev(header); 
	...
	private boolean contains(Position<E> p) {
		DNode<E> nodeToVerify = this.validate(p);
		DNode<E> current = header.getNext();
		while(current != trailer) {
			if(current == nodeToVerify) {
				return true;
			} else {
	...
	public Iterator<E> iterator() {
		return new PLElementBackwardsIterator();
	}
	...
	private class PLElementBackwardsIterator implements Iterator<E> { 
		Iterator<Position<E>> posIterator = new PLIteratorL2F<E>(LinkedPositionalList.this); 
		@Override
		public boolean hasNext() {
